...
namespace App_Ventas.Areas.Inventario.Controllers
{
    public class ImportarController : Controller
    {
        ...
        private void ProcesarHojaCalculo(string RUTA_ARCHIVO_TEMPORAL, int ID_SUCURSAL, ref Cls_Ent_Auditoria auditoria)
        {
        ...
        public List<Cls_Ent_Producto> ObtenerRegistros(DataTable Dt, int ID_SUCURSAL, ref Cls_Ent_Auditoria auditoria)
        {
            ...
            try
            {
                foreach (DataRow row in Dt.Rows)
                {
                    ...
                    item.COD_PRODUCTO = Convert.ToString(row["COD_PRODUCTO"]);
                    item.DESC_PRODUCTO = Convert.ToString(row["DESC_PRODUCTO"]);
                    ...
                    item.FECHA_VENCIMIENTO = Convert.ToString(row["FECHA_VENCIMIENTO"]);
                    if (!string.IsNullOrEmpty(item.FECHA_VENCIMIENTO))
                    item.FLG_VENCE = 1;               
                    item.MARCA = Convert.ToString(row["MARCA"]);
                    item.MODELO = Convert.ToString(row["MODELO"]);
                    item.DETALLE = Convert.ToString(row["DETALLE"]);
